...
window.trackStatusChange = function(id, newStatus) {
    ...
    if (newStatus === 'online' && prev === 'offline') {
        ...
        const stillOffline = [...document.querySelectorAll('.service-card')]
            .some(c => c.dataset.status === 'offline' && !mutedServices.has(c.id.replace('card-', '')));

        if (!stillOffline) stopAlarm();
    }
...
window.toggleMuteService = function(id) {
    ...
    if (mutedServices.has(sid)) {
        ...
        const stillAlarming = [...document.querySelectorAll('.service-card')]
            .some(c => c.dataset.status === 'offline' && !mutedServices.has(c.id.replace('card-', '')));

        if (!stillAlarming) stopAlarm();
    }
}
